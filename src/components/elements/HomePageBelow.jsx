...
const HomePageBelow = () => {
  return (
    ...
    <div
      className="bg-gradient-to-br from-[#100F13] to-[#241a3d] bg-cover w-full"
      style={{
      ...
      <main className="flex justify-center items-center min-h-screen p-4 sm:p-8">
        <section className="relative w-full text-white">
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 ">
            {/* UPDATED: Added justify-items-center to center the cards on all screen sizes */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 justify-items-center">
              {cardData.map((card, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="p-8 rounded-2xl text-center border-purple-400 flex flex-col items-center border justify-center md:h-80 w-48 h-64 md:w-64 "
                  style={{
                ...
                >
                  <div className="border-purple-400 border-4 border-opacity-30 flex my-6 rounded-full h-14 w-14 md:h-20 md:w-20  ">
                    <img
                      src={card.icon}
                      alt={`${card.title} icon`}
                      className="md:w-12 w-8 h-8 md:h-12  mx-auto my-auto"
                    />
                  </div>

                  <p className="font-semibold text-base md:text-lg ">{card.title}</p>
                </motion.div>
              ))}
            </div>
